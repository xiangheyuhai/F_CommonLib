...
void OLED_SPI_ColorTurn(u8 i)
{
	if(i==0)
	{
		OLED_SPI_WR_Byte(0xA6,OLED_CMD);//正常显示
	}
	if(i==1)
	{
		OLED_SPI_WR_Byte(0xA7,OLED_CMD);//反色显示
	}
}
...
void OLED_SPI_DisplayTurn(u8 i)
{
	if(i==0)
	{
		OLED_SPI_WR_Byte(0xC8,OLED_CMD);//正常显示
		OLED_SPI_WR_Byte(0xA1,OLED_CMD);
	}
	if(i==1)
	{
		OLED_SPI_WR_Byte(0xC0,OLED_CMD);//反转显示
		OLED_SPI_WR_Byte(0xA0,OLED_CMD);
	}
}
...
{
	OLED_SPI_WR_Byte(0x8D,OLED_CMD);//电荷泵使能
	OLED_SPI_WR_Byte(0x14,OLED_CMD);//开启电荷泵
	OLED_SPI_WR_Byte(0xAF,OLED_CMD);//点亮屏幕
}
...
{
	OLED_SPI_WR_Byte(0x8D,OLED_CMD);//电荷泵使能
	OLED_SPI_WR_Byte(0x10,OLED_CMD);//关闭电荷泵
	OLED_SPI_WR_Byte(0xAE,OLED_CMD);//关闭屏幕
}
...
void OLED_SPI_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
	   ...
	   for(n=0;n<128;n++)
		 OLED_SPI_WR_Byte(OLED_SPI_GRAM[n][i],OLED_DATA);
  }
}
//清屏函数
void OLED_SPI_Clear(void)
{
	...
	for(i=0;i<8;i++)
	{
	   for(n=0;n<128;n++)
		{
		 OLED_SPI_GRAM[n][i]=0;//清除所有数据
		}
  }
	OLED_SPI_Refresh();//更新显示
}
...
void OLED_SPI_DrawPoint(u8 x,u8 y,u8 t)
{
	...
	m=y%8;
	n=1<<m;
	if(t){OLED_SPI_GRAM[x][i]|=n;}
	else
	{
		OLED_SPI_GRAM[x][i]=~OLED_SPI_GRAM[x][i];
		OLED_SPI_GRAM[x][i]|=n;
		OLED_SPI_GRAM[x][i]=~OLED_SPI_GRAM[x][i];
	}
}
...
void OLED_SPI_DrawLine(u8 x1,u8 y1,u8 x2,u8 y2,u8 mode)
{
	...
	{
		OLED_SPI_DrawPoint(uRow,uCol,mode);//画点
		xerr+=delta_x;
		yerr+=delta_y;
		if(xerr>distance)
		{
...
void OLED_SPI_DrawCircle(u8 x,u8 y,u8 r)
{
    ...
    while(2 * b * b >= r * r)
    {
        OLED_SPI_DrawPoint(x + a, y - b,1);
        OLED_SPI_DrawPoint(x - a, y - b,1);
        OLED_SPI_DrawPoint(x - a, y + b,1);
        OLED_SPI_DrawPoint(x + a, y + b,1);

        OLED_SPI_DrawPoint(x + b, y + a,1);
        OLED_SPI_DrawPoint(x + b, y - a,1);
        OLED_SPI_DrawPoint(x - b, y - a,1);
        OLED_SPI_DrawPoint(x - b, y + a,1);
...
void OLED_SPI_ShowChar(u8 x,u8 y,u8 chr,u8 size1,u8 mode)
{
	...
	for(i=0;i<size2;i++)
	{
		...
		for(m=0;m<8;m++)
		{
			if(temp&0x01)OLED_SPI_DrawPoint(x,y,mode);
			else OLED_SPI_DrawPoint(x,y,!mode);
			temp>>=1;
			y++;
		}
		x++;
...
void OLED_SPI_ShowString(u8 x,u8 y,u8 *chr,u8 size1,u8 mode)
{
	...
	{
		OLED_SPI_ShowChar(x,y,*chr,size1,mode);
		if(size1==8)x+=6;
		else x+=size1/2;
		chr++;
  }
...
void OLED_SPI_ShowNum(u8 x,u8 y,u32 num,u8 len,u8 size1,u8 mode)
{
	...
	for(t=0;t<len;t++)
	{
		temp=(num/OLED_SPI_Pow(10,len-t-1))%10;
			if(temp==0)
			{
				OLED_SPI_ShowChar(x+(size1/2+m)*t,y,'0',size1,mode);
      }
			else
			{
			  OLED_SPI_ShowChar(x+(size1/2+m)*t,y,temp+'0',size1,mode);
			}
  }
}
...
void OLED_SPI_ScrollDisplay(u8 num,u8 space,u8 mode)
{
	...
	while(1)
	{
		if(m==0)
		{
	    OLED_SPI_ShowChinese(128,24,t,16,mode); //写入一个汉字保存在OLED_GRAM[][]数组中
			t++;
		}
		if(t==num)
			{
					...
					for(i=1;i<144;i++)
						{
							for(n=0;n<8;n++)
							{
								OLED_SPI_GRAM[i-1][n]=OLED_SPI_GRAM[i][n];
							}
						}
           OLED_SPI_Refresh();
				 }
...
void OLED_SPI_Init(void)
{
	...
	OLED_SPI_Clear();
	OLED_SPI_WR_Byte(0xAF,OLED_CMD);
}
