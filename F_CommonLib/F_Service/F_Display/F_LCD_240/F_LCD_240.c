...
void LCD_ShowChar_2412(u16 x,u16 y,u8 num,u8 mode,u16 color)
{
    ...
    u8 pos,t;
	u16 x0=x;
    if(x>LCD_W-24||y>LCD_H-24)return;	    //设置窗口
	num=num-' ';//得到偏移后的值
	LCD_Address_Set(x,y,x+12-1,y+24-1);      //设置光标位置
	if(!mode) //非叠加方式
	{
		for(pos=0;pos<24;pos++)
		{
			temp=asc2_2412[(u16)num*24+pos];		 //调用2412字体
			for(t=0;t<12;t++)
		    {
		        if(temp&0x01)LCD_WR_DATA(color);
				    else LCD_WR_DATA(BACK_COLOR);
...
void LCD_ShowString_2412(u16 x,u16 y,const u8 *p,u16 color)
{
    while(*p!='\0')
    {
        if(x>LCD_W-24){x=0;y+=24;}
        if(y>LCD_H-24){y=x=0;LCD_Clear(RED);}
        LCD_ShowChar_2412(x,y,*p,0,color);
        x+=12;
        p++;
    }
}
